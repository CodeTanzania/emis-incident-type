...
const uniqueIndex =
  ({ nature: 1, family: 1, name: 1, code: 1, cap: 1 });
...
IncidentTypeSchema.methods.preValidate = function preValidate(done) {
  ...
  // ensure nature
  if (_.isEmpty(this.nature)) {
    this.nature = NATURE_OTHER;
  }

  // ensure family
  if (_.isEmpty(this.family)) {
    this.family = FAMILY_OTHER;
  }
  ...
  if (this.name && _.isEmpty(this.code)) {
    ...
    // prepare given code
    let code =
      _.compact(([this.nature, this.family, this.name.split(' ')]));
  ...
  // ensure cap code
  if (_.isEmpty(this.cap)) {
    this.cap = CAP_CATEGORY_OTHER;
  }
