...
const { FAMILIES, NATURES, CAP_CATEGORY_OTHER } = constants;
...
const IncidentTypeSchema = new Schema({
  ...
  /**
   * @name icon
   * @description An icon(url, base64, svg etc) used to differentiate
   * incident types visually.
   *
   * @type {object}
   * @property {object} type - schema(data) type
   ...
   * this incident type(or disaster).
   *
   * @type {object}
   * @property {object} type - schema(data) type
   * @property {boolean} trim - force trimming
...
IncidentTypeSchema.methods.preValidate = function preValidate(done) {
  ...
  // ensure cap code
  if (this.code && !this.code.cap) {
    this.code.cap = CAP_CATEGORY_OTHER;
  }
