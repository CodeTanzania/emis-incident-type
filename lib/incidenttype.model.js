...
/* cap categories */
const CAP_CATEGORY_OTHER = 'Generic';
...
const IncidentTypeSchema = new Schema(
  {
     ...
     *
     * If not specified it will be generated from first letters of nature, 
     * family and name of incident type(or disaster).
     *
     * @type {object}
     * @property {object} type - schema(data) type
     * @property {boolean} trim - force trimming
     ...
     * @description Human readable typical or noticeable features associated
     *  with an incident type(or disaster) event.
     *
     * @type {object}
     * @property {object} type - schema(data) type
     * @property {boolean} default - default value set when none provided
    ...
    /**
     * @name causes
     * @description Human readable possible actions or conditions that may lead
     *  to an incident type(or disaster) event.
     *
     * @type {object}
     * @property {object} type - schema(data) type
     * @property {boolean} default - default value set when none provided
...
IncidentTypeSchema.methods.preValidate = function preValidate(done) {
  ...
  if (this.name && _.isEmpty(this.code)) {
    ...
    // prepare given code
    let code = _.compact(
      [this.nature, this.family].concat(this.name.split(' '))
    );

    // generate given code
    code = _.reduce(
      code,
      function generateGivenCode(code, word) {
 ...
 * @description prepare incidenttype seeding upsert criteria
 * @param {Object} seed plain object incidenttype seed
 * @return {Object} criteria used to upsert incidenttype
 *
 * @author lally elias <lallyelias87@gmail.com>
 * @since 1.5.0
 * @version 0.1.0
 * @public
 */
IncidentTypeSchema.statics.prepareSeedCriteria = seed => {
  ...
  // return incidenttypes upsert criteria
  return criteria;
};
